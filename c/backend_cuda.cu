 ...
 * has a ceiling only, up is bounded both ways) -- matches glm53.c's own
 * swiglu_clamped() on the host, run in place on the device so the gate/up
 * intermediates never have to leave VRAM for it. */
 ...
 * where the text MLP is not plain SiLU -- using the unclamped kernel there
 * would silently produce a wrong (if plausible-looking) answer. One
 * host->device transfer for x, one device->host transfer for y -- gate/up/
 * down and the activation in between never touch the host, same as the
 * unclamped version above. Routed experts on this engine are always fmt=4
 * (int4-g64); the fmt==6 (e8 rotation) case coli_cuda_expert_mlp doesn't
 * handle either is intentionally not handled here either. */
