...
def main():
    ...
    ap.add_argument("--mtp", action="store_true",
        help="download/convert ONLY the MTP head (model.layers.<n_layers>.*) -> out-mtp-*.safetensors")
    ap.add_argument("--indexer", action="store_true",
        help="extract ONLY the DSA lightning indexer weights -> out-idx-*.safetensors. WARNING: "
             "indexer tensors are scattered across ~all shards: this re-downloads the entire repo (~756 GB "
             "of traffic) to keep just a few GB. Resumable shard by shard. --ebits 8 recommended.")
    a = ap.parse_args()
    if a.ebits is None:
    ...
    try: fcntl.flock(lock, fcntl.LOCK_EX | fcntl.LOCK_NB)
        ...
        print("ERROR: another converter is already working on this outdir. Exiting."); return
    ...
    def _download_single(url, fn, out, part, expected):
        ...
        print(f"    [dl] {fn}: {sz/1e9:.2f} GB in {dt/60:.1f} min "
              f"({sz/dt/1e6:.1f} MB/s avg, {nres} riprese/resumes)", flush=True)
        return out
    ...
    for att in range(999):
        try:
            ...
            w = min(60, 5*(att+1)); print(f"repo_info KO ({type(ex).__name__}): retrying in {w}s", flush=True); _t.sleep(w)
    ...
    for i, sh in enumerate(shards):
        if free_gb(a.outdir) < a.min_free_gb:
            print(f"STOP: free space < {a.min_free_gb} GB. Free up space and rerun (resumes)."); break
        outp = os.path.join(a.outdir, f"out-{i:05d}.safetensors")
        if os.path.exists(outp): continue                 # already done -> resumable
        print(f"[{i+1}/{len(shards)}] downloading {sh} (free {free_gb(a.outdir):.0f} GB)...", flush=True)
        p = download_retry(a.repo, sh, tmp)
